...
function buildMusicPlan(source: AssemblyMusicSource, totalDurationS: number, fadeSeconds: number): MusicPlan {
  ...
  return { storage_path: source.storage_path, start_s: start, end_s: end, loop, fade_in_s: fade, fade_out_s: fade };
}
...
export function buildAssemblyPlan(params: BuildAssemblyPlanParams): AssemblyPlan {
  ...
  for (let i = 0; i < params.scenesByPosition.length; i++) {
    ...
    clips.push({ scene_id: scene.id, role: "scene", storage_path: storagePath, trim_head_frame: trimHead });
  }
  ...
  clips.push({ scene_id: null, role: "outro", storage_path: params.outroClipStoragePath, trim_head_frame: false });
  ...
  const music = params.music ? buildMusicPlan(params.music, totalDurationS, params.fadeSeconds ?? 1.5) : null;
