 ...
 *
 * Branded end-frame image: an `endframe_render` job (satori->resvg,
 * src/lib/jobs/endframe.ts) from logo + outro_tagline (client default, editable);
 ...
 * lacks end-frame support) — the handler extracts the clip's last frame via
 * ffmpeg; end = branded end-frame image). Else a deterministic
 * crossfade-to-endframe (no provider call, cost $0). Both routes are
 * dispatched as an `outro_gen` job (src/lib/jobs/run.ts branches on
...
import type { ReelModelContext } from "@/src/skills/model-prompt/constraints";
import { getCurrentPromptVersion } from "@/src/lib/versioning";
...
async function load(ctx: StageContext): Promise<StageState> {
  ...
  if (data) {
    ...
    stored = ((current?.metadata as { target_model?: string } | null | undefined)?.target_model) ?? null;
  }

  return {
    stage: "outro",
    data: { reelConfig, prompt_staleness: promptStaleness(stored, await outroTargetModel(ctx)) },
  };
}
 ...
 */
const OUTRO_SEED_PROMPT = "Smooth, elegant motion transitioning the last shot into a clean branded end card.";

/**
 * The outro clip is generated by whichever b-roll model resolveOutroRoute picks,
 * so the prompt is optimised for that one. A crossfade route generates no clip
 * at all, so there is no model to optimise for.
 */
async function outroTargetModel(ctx: StageContext): Promise<TargetModel | null> {
  return (await outroTarget(ctx)).target;
}

/**
 * The outro's target model AND the constraint context its prompt is written
 * against.
 *
 * `uses_end_frames` is unconditionally true here: the model route only exists
 * when `supports_end_frame` is true (that is what resolveOutroRoute checks),
 * and the outro clip is by definition an interpolation from the last scene's
 * end frame to the branded end card. So on the Veo 3.1 family this clip always
 * renders at the full 8s — worth the prompt knowing, because
 * `reel_config.outro_seconds` defaults to 2 and the difference is generated,
 * billed, and then trimmed away.
 */
async function outroTarget(ctx: StageContext): Promise<{ target: TargetModel | null; modelCtx?: ReelModelContext }> {
  const reelConfig = await getReelConfig(ctx.supa, ctx.reelId);
  const route = resolveOutroRoute(reelConfig, supportsEndFrameLookupFor(ctx.adapters, reelConfig.veo_variant));
  if (route.kind === "crossfade") return { target: null };
  return {
    target: targetModelFor(route.provider, reelConfig.veo_variant ?? undefined),
    modelCtx: reelModelContext(reelConfig, { uses_end_frames: true }),
  };
}
...
export async function ensureOutroPrompt(ctx: StageContext): Promise<{ promptId: string; versionId: string }> {
  ...
  const { target, modelCtx } = await outroTarget(ctx);
...
async function enqueueOutroClip(
  ...
  const job = await ctx.jobs.enqueue({
    ...
    // N2: without this, src/lib/jobs/reconcile.ts's double-charge guard
    ...
    payload: {
      ...
      // produced one; else the handler extracts the last frame of its clip
  ...
  // Crossfade route finishes here; the model route comes back
  // awaiting_provider and is finished by reconcilePendingJobs() on a later
  // read of this stage.
  return { assetId, job: await runJobInline(ctx, job) };
}
...
async function process(input: OutroInput, ctx: StageContext): Promise<OutroOutput> {
  ...
  if (input.custom_end_frame_storage_path) {
    ...
    // Persist the choice BEFORE rendering: the render is now inline, so a
    // failure here used to be impossible but can now throw. Writing first
    // keeps end_frame_mode/outro_tagline (and the outro clip below) exactly as
    // they were when this only enqueued a job.
    await ctx.supa
      .from("reel_config")
...
export function createOutroReviewHooks(ctx: StageContext): ReviewHooks {
  return {
    async redoPrompt(promptId) {
      ...
      const brand = await getBrandContext(ctx.supa, ctx.clientId);
      // Reads the CURRENT outro route, so a redo is also the "re-optimise for
      // the model I switched to" action.
      const { target, modelCtx } = await outroTarget(ctx);
      const text = await ctx.skills.brandStyleLock(OUTRO_SEED_PROMPT, brand, undefined, target ?? undefined, modelCtx);
